...
draws_hyper <- readRDS('poststratification_chat_pooling.rds')$draws()
# ----------------------------------------------------
# 2. 转换为标准后验对象格式
# ----------------------------------------------------
draws_norm_obj <- as_draws_array(draws_norm)
draws_hyper_obj <- as_draws_array(draws_hyper)
...
variables(phi_draws_hyper) <- names_31
...
phi_hyper_mat <- as_draws_matrix(phi_draws_hyper)        # 4000 × 26


diff_mat <- phi_norm_mat - phi_hyper_mat  # 4000 × 26
# ----------------------------------------------------
# 8. 转为长数据（ggplot绘图必需）
# 计算后验差异的统计量
