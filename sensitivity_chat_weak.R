...
# 直接替换成数字
psframe$prov <- as.integer(as.factor(psframe$unit))
province$unit <- as.integer(as.factor(province$prov))
names(table(psframe$unit)) == names(table(province$prov))
unique(province[,c('unit','prov')])

df$unit <- province$unit[match(df$prov, province$prov)]
summary(province)
...
four_way_array <- array(data = psframe$freq,
                        ...
                        dimnames = list(
                          ...
                          urban = c("1", "2") ))
# alpha ~ normal(0, 5);
# beta_* ~ normal(0, 5);        // ⬅️ 更宽
# sigma_gamma ~ exponential(0.5); // ⬅️ 更大方差
# sigma_area  ~ exponential(0.5);
# sigma_state ~ exponential(0.5);
...
data {
  ...
  array[31] real ratio_urban_rural, per_capita, college, growth_of_gdp;
  array[31] real<lower=0, upper=1> rate_coresid, migration, mot_il,
  fat_farmer, status_yuv;
  
  // area mapping
  array[31] int<lower=1, upper=4> area;
  array[31, 4, 2, 2] int<lower=0> P;
...
model {
  ...
  // scales
  sigma_gamma ~ exponential(.5);
  sigma_area  ~ exponential(.5);
  sigma_state ~ exponential(.5);
...
fit.bb$save_object("poststratification_chat_weak.rds")