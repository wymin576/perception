...
# 模型2：仅包含 26 个大样本省份（已剔除海南、西藏、新疆）
draws_weak <- readRDS('poststratification_chat_weak.rds')$draws()
# ----------------------------------------------------
# 2. 转换为标准后验对象格式
# ----------------------------------------------------
draws_norm_obj <- as_draws_array(draws_norm)
draws_weak_obj <- as_draws_array(draws_weak)
...
phi_draws_weak <- subset_draws(draws_weak_obj, 
                                variable = paste0("phi[", 1:31, "]"))
...
variables(phi_draws_weak) <- names_31
...
phi_weak_mat <- as_draws_matrix(phi_draws_weak)        # 4000 × 26


diff_mat <- phi_norm_mat - phi_weak_mat  # 4000 × 26
# ----------------------------------------------------
# 8. 转为长数据（ggplot绘图必需）
# 计算后验差异的统计量
