...
write_xlsx(loadings_df, "pca_loadings.xlsx")
...
# saveRDS(fit$draws(), "base_model_draws.rds", compress = "xz")


# -----------------------------------------figures--------------
# 匹配多个模式（或关系）
patterns <- c("beta_",'hukou' ,'gender',"gamma\\[","zeta\\[")
pattern_combined <- paste(patterns, collapse = "|")
# "apple|grape"
var_meta <- fit$metadata()$variables
vars <- var_meta[grep(pattern_combined,var_meta)]

result <- fit$summary(variables = vars) 
write.csv(result,'result_opt.csv')
# --------------------------------------------------------
# ************************fig***************
# --------------------------------------------------------
result$variable
data <-  result %>%  select(variable,median,q5,q95) %>% 
  mutate(var.name = case_when(variable == "beta_1"~ 'RC1',   
                              variable == "beta_2"~ 'RC2',
                              variable == "beta_3"~ 'RC3', 
                              variable == "beta_4"~ 'RC4', 
                              variable == "beta_5"~ 'RC5', 
                              variable == "gender_eff" ~ 'Gender (ref: Male)',
                              variable == "hukou_eff" ~ 'Hukou (ref: Urban)',
                              variable == "gamma[1]" ~ 'Aged 20–24',
                              variable == "gamma[2]" ~ 'Aged 25–29',
                              variable == "gamma[3]" ~ 'Aged 30–34',
                              variable == "gamma[4]" ~ 'Aged 35–39',
                              variable == "zeta[1]" ~ 'East region',
                              variable == "zeta[2]" ~ 'Middle region',
                              variable == "zeta[3]" ~ 'West region',
                              variable == "zeta[4]" ~ 'Northeastern region')
  ) |> na.omit()

min(data$q5);max(data$q95)

graphics.off()
# 简化版（如果喜欢干净风格）
ggplot(data, aes(y = fct_rev(fct_inorder(var.name)))) +
  geom_errorbar(
  ...
  geom_vline(xintercept = 0, 
             ...
             alpha = 1.5) +
  # 顶部边框线
  geom_hline(yintercept = length(unique(data$var.name)) + 0.5,
             color = "black",
             linewidth = 0.8) +
  # 底部边框线（可选）
  ...
  scale_x_continuous(
    name = NULL,
    limits = c(min(data$q5)*1.1,max(data$q95)*1.1),
    breaks = seq(min(data$q5)*1.1,max(data$q95)*1.1, 0.1),
  ...
  theme(
    ...
    # X轴（底部）
    axis.title.x.bottom = element_text(size = 6, color = "black", margin = margin(t = 10)),
...
# 提取后验样本
draws_df <- fit$draws(variables = c("y_sum", "y_gt"),
                      format = "data.frame")

# 计算y_gt的后验概率
posterior_prob_y_gt <- mean(draws_df$y_gt)

summary(draws_df$y_sum)
# 绘制y_sum的直方图并添加概率文本
ggplot(draws_df, aes(x = y_sum)) +
  geom_histogram(bins = 30, fill = "skyblue", color = "black") +
  geom_vline(aes(xintercept = sum(df$up)), 
             linetype = "dashed", color = "red") +
  labs(title = paste0("p = ", round(posterior_prob_y_gt, 3)),
       x = "T(y)",
       y = NULL
  ) + 
  scale_x_continuous(limits = c(min(draws_df$y_sum)-50, 
                                max(draws_df$y_sum)+50)) +
  theme_minimal() +
  theme(
...
write.csv(state_tot,'state_perception.csv')
# 绘图
...
ggplot(state_tot, aes(x = reorder(prov_en, median), y = median)) +
  ...
  # 设置y轴为百分率格式
  scale_y_continuous(labels = percent, limits = c(0.3, 0.75)) +
...
# 如果需要保存
ggsave("fig3_poststratification.png", 
